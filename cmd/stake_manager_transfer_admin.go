...
func stakeManagerTransferAdminCmd() *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg, err := config.LoadConfig[config.ConfigTransferAdmin](configPath)
			if err != nil {
				return err
			}
		...
		Out:
			...
			instructions := []solana.Instruction{
				lsd_program.NewTransferStakeManagerAdminInstruction(
					solana.MustPublicKeyFromBase58(cfg.NewAdminAccount),
