...
func stakeManagerInitCmd() *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			stakeManager, _, err := solana.FindProgramAddress([][]byte{utils.StakeMangerSeed, adminAccount.PublicKey().Bytes(), []byte{0}}, lsdProgramID)
			if err != nil {
				return err
			}
			lsdTokenMint, _, err := solana.FindProgramAddress([][]byte{utils.TokenMintSeed, adminAccount.PublicKey().Bytes(), []byte{0}}, lsdProgramID)
			if err != nil {
				return err
			}
